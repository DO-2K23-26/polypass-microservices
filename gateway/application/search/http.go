// filepath: gateway/application/search/http.go
package search_http
...
func (c *Controller) handleCredentials(ctx *fiber.Ctx) error {
	...
	tagIds := ctx.Query("tag_ids")
	var tags []string
	if tagIds != "" {
		tags = append(tags, tagIds)
	}
	req := &pb.SearchCredentialsRequest{SearchQuery: q, FolderId: ctx.Query("folder_id"), TagIds: tags, Limit: int32(limit), Page: int32(page), UserId: ctx.Query("user_id")}
